            ...
            @if (!$barangKeluar)
                <form method="post" action="{{ route('barang-keluar.store') }} " class="uppercase"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <x-bladewind::select name="kode_barang" required="true" :data="$barangMasuk"
            ...
            fetch(`{{ route('barang-keluar.search') }}?search=${searchQuery}`, {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('jumlah-laptop-masuk').textContent =
                            `(Jumlah laptop masuk: ${data.data.jlh_laptop})`;
                    } else {
